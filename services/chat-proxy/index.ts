...
const DEFAULT_PYTHON_API_URL = "http://chart-processor:8000";
...
export async function handleChatRequest(req: Request): Promise<Response> {
  ...
  try {
    ...
    const delphiProxyUrl = env("DELPHI_PROXY_URL") || "http://delphi-proxy:8787/external-db-proxy";
...
function isIndexChartNormalizationFallbackEnabled(): boolean {
  const raw = env("INDEX_CHART_NORMALIZATION_FALLBACK");
  if (!raw) return true;
...
async function generateChartFromPython(
  ...
  const pythonApiUrl = env("PYTHON_API_URL") || DEFAULT_PYTHON_API_URL;
  const internalToken = env("PYTHON_INTERNAL_API_TOKEN") || env("INTERNAL_API_TOKEN") || env("PYTHON_API_TOKEN");
